...
def fick_mass(c, length, width, key):
    m = 0.
...
def time_iteration(c_init_list, n_t, dt, dr, key, key_sch='implicit'):
    c_app = np.zeros(n_t)
    ...
    mass_list[0] = fick_mass(c_matrix[0], length, width, key)
    c_app[0] = 0.
    for i in range(1, n_t):
        c_bound = c_app[i - 1]
        ...
        if volume < N * 4 / 3 * np.pi * (R)**3 and key == 'sphere' :
            ...
            mass_list[i] = fick_mass(c_matrix[i], length, width, key)
